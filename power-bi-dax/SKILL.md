...
## ⚠️ DAX execution is blocked on offline (TMDL folder) connections

When the active connection is a TMDL folder (`pbi connect ./Model.SemanticModel` rather than a live Desktop/AAS/Fabric instance), all query operations fail with:

> `"DAX query operations are not supported on offline connections"`

This affects `pbi dax execute`, `pbi dax validate`, `pbi dax explain`, `pbi dax clear-cache`, and any scripted use of the `dax_query_operations` MCP endpoint. The reason: there is no VertiPaq engine running to execute `EVALUATE`; the TMDL folder is a declarative representation only.

**Implications when building a model offline:**
- You can write measure DAX and see it saved to `.tmdl`, but you cannot prove it evaluates correctly until you open in Power BI Desktop.
- Measure-name collisions with columns (see **power-bi-modeling** skill) are caught only at import time into a live engine.
- Formula correctness, division-by-zero guards, filter-context bugs, and `USERELATIONSHIP` mistakes all slip through offline.

**Validation workflow:**

```bash
# 1. Build structure offline (fast loop, many iterations)
pbi connect ./Retail.SemanticModel
pbi measure create "Net Sales" -e "SUM(FactSales[NetAmountILS])" -t FactSales
pbi database export-tmdl ./Retail.SemanticModel/definition/

# 2. Open Retail.pbip in Power BI Desktop — this boots a live mini-engine
#    and runs M partitions to populate the data model

# 3. Reconnect pbi-cli to the live Desktop instance (auto-detects the port)
pbi disconnect
pbi connect

# 4. Now DAX queries work
pbi dax execute "EVALUATE ROW(\"Net Sales\", [Net Sales])"
pbi dax validate "SUM(FactSales[NetAmountILS])"
```

**Planning tip:** write sanity-check queries in a `.dax` file while building offline. Run them as a batch against the live Desktop instance once data is populated — catches measure name errors and formula mistakes in one pass.
